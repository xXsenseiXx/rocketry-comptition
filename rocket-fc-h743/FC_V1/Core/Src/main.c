...
#include "mpu6500.h"
#include "usbd_cdc_if.h"  // <--- Adds the CDC_Transmit_FS function
#include <string.h>       // <--- Adds strlen()
#include <stdio.h>        // <--- Adds sprintf()
...
int main(void)
{
  ...
  Task_IMUHandle = osThreadNew(StartTask02, NULL, &Task_IMU_attributes);

  /* creation of Task_Debug */
  Task_DebugHandle = osThreadNew(StartTask03, NULL, &Task_Debug_attributes);
...
void StartTask02(void *argument)
{
  /* USER CODE BEGIN StartTask02 */
	  /* Initialize the sensor here, not in debug task! */
  if(MPU6500_Init() != 1) {
	  // If it fails, blink the LED rapidly and suspend the task
	  while(1) {
...
void StartTask03(void *argument)
{
  ...
  for(;;)
  {
	...
	// Format your data
	sprintf(usb_buffer, "%d,%d,%d,%d,%d,%d\r\n",
			accel_raw[0], accel_raw[1], accel_raw[2],
			gyro_raw[0], gyro_raw[1], gyro_raw[2]);

	// Transmit over USB Type-C
